...
public hallinta(){
     ...
     */    
  	public static void raportti (Connection connection, int id, String aPaiva, String lPaiva) throws SQLException, Exception { // tietokantayhteys välitetään parametrina
            ...
            String sql2 = "SELECT concat(etunimi, ' ', sukunimi) AS nimi FROM asiakas WHERE asiakas_id IN (SELECT asiakas_id FROM varaus WHERE varattu_alkupvm BETWEEN CAST(? AS DATE) AND CAST(? AS DATE) AND varaus_id IN(SELECT varaus_id FROM varauksen_mokit WHERE toimipiste_id = ?)) GROUP BY asiakas_id;";
            // hakee paivamaarat valitulta ajalta tietyltä ajalta ja tietyn toimipisteID:n mukaan, lajittelu asiakas_id perusteella 
            String sql3 = "SELECT DATE(varattu_alkupvm) AS varattu_alkupvm, DATE(varattu_loppupvm) AS varattu_loppupvm FROM varaus WHERE varaus_id IN (SELECT varaus_id FROM varauksen_mokit WHERE varaus_id IN(SELECT varaus_id FROM varaus WHERE varattu_alkupvm BETWEEN CAST(? AS DATE) AND CAST(? AS DATE) AND toimipiste_id = ?)) GROUP BY asiakas_id;" ;
            // hakee palveluiden nimen, hinnan ja lukumaaran valitulta ajalta ja tietyn toimipisteID:n mukaan, lajittelu asiakas_id:n ja mokki_id:n perusteella
            String sql4 = "SELECT concat(mokki.nimi, ' hinta ', mokki.hinta, ' paivat ', varauksen_mokit.lkm) AS nimi_hinta_paiva, varaus.asiakas_id FROM varaus, mokki, varauksen_mokit WHERE varauksen_mokit.varaus_id = varaus.varaus_id AND mokki.mokki_id = varauksen_mokit.mokki_id AND varaus.asiakas_id IN (SELECT varaus.asiakas_id FROM varaus WHERE varaus.varattu_alkupvm BETWEEN CAST(? AS DATE) AND CAST(? AS DATE) AND varaus.toimipiste_id = ?) GROUP BY varaus.asiakas_id, varauksen_mokit.mokki_id;";
            // hakee palveluiden kokonaissumman valitulta ajalta ja tietyn toimipisteID:n mukaan, lajittelu asiakas_id perusteella
            String sql5 = "SELECT SUM(mokki.hinta * varauksen_mokit.lkm) AS hinta FROM varaus, mokki, varauksen_mokit WHERE varauksen_mokit.varaus_id = varaus.varaus_id AND mokki.mokki_id = varauksen_mokit.mokki_id AND varaus.asiakas_id IN (SELECT varaus.asiakas_id FROM varaus WHERE varaus.varattu_alkupvm BETWEEN CAST(? AS DATE) AND CAST(? AS DATE) AND varaus.toimipiste_id = ?) GROUP BY varaus.asiakas_id;";
		...
		if (tulosjoukko == null) {
            ...
            // luodaan muistio tiedosto
            File raportti = new File("Mokki_" + tulos  + ".txt");
    ...
    public static void raportti2 (Connection connection, int id, String aPaiva) throws SQLException, Exception { // tietokantayhteys välitetään parametrina
                ...
                String sql = "SELECT nimi" 
                            + " FROM toimipiste WHERE toimipiste_id = ?";
                // hakee asiakkaiden nimen valitulta ajalta ja tietyn toimipisteID:n mukaan, lajittelu asiakas_id perusteella
                String sql2 = "SELECT concat(etunimi, ' ', sukunimi) AS nimi FROM asiakas WHERE asiakas_id IN (SELECT asiakas_id FROM varaus WHERE DATE(varaus.varattu_alkupvm) = ? AND varaus_id IN(SELECT varaus_id FROM varauksen_mokit WHERE toimipiste_id = ?)) GROUP BY asiakas_id;";
                // hakee paivamaaran valitulta ajalta tietyltä ajalta ja tietyn toimipisteID:n mukaan, lajittelu asiakas_id perusteella  
                String sql3 = "SELECT DATE(varattu_alkupvm) AS varattu_alkupvm FROM varaus WHERE varaus_id IN (SELECT varaus_id FROM varauksen_mokit WHERE varaus_id IN(SELECT varaus_id FROM varaus WHERE DATE(varaus.varattu_alkupvm) = ? AND toimipiste_id = ?)) GROUP BY asiakas_id;" ;
                // hakee palveluiden nimen, hinnan ja lukumaaran valitulta ajalta ja tietyn toimipisteID:n mukaan, lajittelu asiakas_id:n ja mokki_id:n perusteella
